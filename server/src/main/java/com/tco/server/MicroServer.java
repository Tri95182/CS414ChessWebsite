...
class MicroServer {
    ...
    private void processRestfulAPIrequests() {
        path("/api", () -> {
            ...
            post("/login", (req, res) -> processHttpRequest(req, res, RegisterRequest.class));
            post("/creategame", (req, res) -> processHttpRequest(req, res, CreateGameRequest.class));
            post("/activeGames", (req, res) -> processHttpRequest(req, res, ActiveGamesRequest.class));
    ...
    private String processHttpRequest(spark.Request httpRequest, spark.Response httpResponse, Type requestType) {
        ...
        try {
            ...
            if (!requestObj.getRequestType().equals("config")) {
                return buildJSONResponse(requestObj, httpRequest, requestType);
            }
    ...
    private String buildJSONResponse(Request requestObj, spark.Request httpRequest, Type requestType) throws BadRequestException {
        switch (requestObj.getRequestType()) {
            case "register":
                RegisterRequest registerRequest = new RegisterRequest();
                registerRequest.buildResponse(httpRequest);
                requestObj = registerRequest;
                break;
            case "login":
                LoginRequest loginRequest = new LoginRequest();
                loginRequest.buildResponse(httpRequest);
                requestObj = loginRequest;
                break;
            case "createGame":
                CreateGameRequest createGameRequest = new CreateGameRequest();
                createGameRequest.buildResponse(httpRequest);
                requestObj = createGameRequest;
                break;
            case "activeGames":
                ActiveGamesRequest activeGamesRequest = new ActiveGamesRequest();
                activeGamesRequest.buildResponse(httpRequest);
                requestObj = activeGamesRequest;
                break;
            default:
                throw new IllegalStateException("Unexpected value: " + requestType);
        }
        String responseBody = new Gson().toJson(requestObj);
